...
foreach ($slugs as $slug) {
  $slugMethod = "get{$slug}Slug";
  $t->comment("->{$slugMethod}()");
  $job = Doctrine_Core::getTable('JobeetJob')->createQuery()->fetchOne();
  $t->is(call_user_method($slugMethod, $job), Jobeet::slugify(call_user_method("get{$slug}", $job)), "->{$slugMethod}() return the slug for the {$slug}");
}
